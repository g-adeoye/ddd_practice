...
async def test_dispatch_times_out():
    """
    Handler that runs longer than the timeout must raise a TimeoutError.
    This is the mechanism that prevents hung jobs from blocking workers.
    """
    from services.worker_service import _dispatch

    async def slow_payload():
        return await asyncio.wait_for(
            _dispatch({"type": "send_email", "to": "x@y.com"}),
            timeout=0.001,  # far below the 0.1s sleep in the send_email handler
        )
...
async def test_dispatch_unknown_type_raises():
    "Unkown job type must raise ValueError, not silently succeed."
    from services.worker_service import _dispatch
...
def test_backoff_increases_with_retry_count():
    ...
    """
    from services.worker_service import _backoff_seconds
