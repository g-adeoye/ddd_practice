async def test_backpressure_503_when_queue_full(client, redis):
    """
    When total queue depth exceeds HIGH_WATERMARK, the API must return
    503 with a Retry-After header - not accept the job silently.

    This proves backpressure works: the system signals overload explicitly
    rather than accepting work it cannot handle.
    """
    from app.config import get_settings
    from services.queue_service import _ensure_consumer_group
    from services.scheduler import QUEUE_NAMES
...
async def test_priority_distribution_matches_weights(client, redis):
    ...
    from services.scheduler import get_weights, pick_queue
...
async def test_job_appears_in_pending_after_submission(client, redis):
    ...
    as a real message, not just a DB record.

    This shows the enqueue step actually wrote to the Stream.
    """
    response = await client.post(
        "/jobs",
        json={
...
async def test_queue_metrics_reflect_submitted_jobs(client, redis):
    ...
    for priority in ["high", "normal"]:
        await client.post(
            "/jobs", json={"payload": {"type": "generate_report"}, "priority": priority}
        )
...
async def test_low_watermark_resumes_acceptance(client, redis):
    """
    Once the queue drains below LOW_WATEMARK, the system must
    ...
    """
    from app.config import get_settings
    from services.queue_service import BACKPRESSURE_STATE_KEY, _ensure_consumer_group
    from services.scheduler import QUEUE_NAMES
