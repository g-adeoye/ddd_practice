...
from services.scheduler import QUEUE_NAMES, get_weights, pick_queue
...
async def enqueue(
    ...
    if in_backpressure:
        ...
        await redis.set(BACKPRESSURE_STATE_KEY, "1", ex=5 * 60)  # 5 min TTL as safety
        raise BackPressureError(
...
async def dequeue(redis: Redis, consumer_name: str) -> QueueMessage | None:
    ...
    results = await redis.xreadgroup(
        ...
        count=1,
        block=2000,  # wait up to 2 seconds if the stream if empty
    )
...
async def acknowledge(redis: Redis, stream: str, message_id: str) -> None:
    ...
    Removes if from the PENDING entries list.
    """
...
async def get_pending_messages(
    ...
    return cast(
        ...
        await redis.xpending_range(
            stream, CONSUMER_GROUP, min="-", max="+", count=100, idle=min_idle_ms
        ),
    )
