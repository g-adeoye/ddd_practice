...
from infrastructure.metrics import zombie_jobs_reaped_total
from services.queue_service import CONSUMER_GROUP, acknowledge, enqueue
...
VISIBILITY_TIMEOUT_MS: dict[str, int] = {
...
async def recover_pending_messages(redis: Redis) -> int:
    ...
    priority-specific visibility timeout. Re-enqueue them so another worker can process them.
    ...
    for priority, stream in QUEUE_NAMES.items():
        timeout_ms = VISIBILITY_TIMEOUT_MS[priority]
        recovered_this_stream = 0  # <- track per stream

        try:
            pending = await redis.xpending_range(
                stream, CONSUMER_GROUP, min="-", max="+", count=100, idle=timeout_ms
            )
            ...
            for entry in pending:
                ...
                if claimed and claimed[1]:
                    ...
                    for msg_id, data in messages:
                        ...
                        recovered += 1
                if (
                    recovered_this_stream
                ):  # <- now inside the for loop, after the try block
                    logger.info(
                        "repear_recovered_messages",
                        stream=stream,
...
async def reap_zombie_jobs(db: AsyncSession, redis: Redis) -> int:
    ...
    Returns the count of zombies repead.
    """
