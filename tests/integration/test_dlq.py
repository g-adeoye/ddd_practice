...
async def test_exhausted_job_appears_in_dlq(client, redis, db):
    """
    A job that fails and has max_retries = 0 must land in the DLQ stream,
    not silently disappear.
    """
    from services.queue_service import ensure_all_consumer_groups
    ...
    response = await client.post(
        "/jobs",
        json={
            "payload": {"type":"always_fail"},
            "priority": "normal",
            "max_retries": 1
        }
    )
    
    weights_patch = await client.patch(
        "/queues/weights",
        json={
            "critical": 0,
            "high": 0,
    ...
    assert weights_patch.status_code == 200
    assert response.status_code == 202
    
    # Simulate the worker exhausting retries by calling mark_failed directly
    from services.queue_service import get_dlq_depth
    from services.worker_service import process_one
    
    dlq_depth_before = await get_dlq_depth(redis)
    
    # Push directly to DLQ to test the stream mechanism
    await process_one(redis, db, consumer_name="test_worker")
...
async def test_dlq_depth_appears_in_metrics(client, redis):
    ...
    from services.queue_service import enqueue_dlq
...
async def test_replay_dlq_message(client, redis):
    ...
    """
    from services.queue_service import enqueue_dlq, get_dlq_depth
