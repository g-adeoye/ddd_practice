...
from schemas.job import QueueDepthResponse, WeightResponse, WeightUpdateRequest
...
async def replay_dlq(message_id: str, redis: RedisDep) -> dict[str, Any]:
    ...
    Use this after investigating and fixing root cause of a failure.
...
async def get_weights_endpoint(redis: RedisDep) -> WeightResponse:
    ...
    """
    from services.scheduler import get_weights
...
async def update_weights(body: WeightUpdateRequest, redis: RedisDep) -> WeightResponse:
    ...
    return WeightResponse(
        critical=body.critical, high=body.high, normal=body.normal, source="redis"
    )
